...
请读取屏幕上显示的以下六个参数数值：
1. tension (表/界面张力)：屏幕上方最大的数字，单位 mN/m。可能带有负号，通常有3位小数。
2. temperature (温度)：带 °C 单位的数值。若显示 N/A 则为 null。
3. upper_density (上层密度)：单位 g/cm3。
4. lower_density (下层密度)：单位 g/cm3。
5. rise_speed (上升速度)：单位 mm/min。
6. fall_speed (下降速度)：单位 mm/min。

【重要】严格按以下 JSON 格式输出，不要输出任何其他内容（不要Markdown代码块，不要解释）：
...
- 屏幕上的 F: - / + 是按钮操作，不是数值的正负号，请勿混淆。"""
    ...
    conn.commit()
    conn.close()
    print("F5 Prompt updated successfully.")
