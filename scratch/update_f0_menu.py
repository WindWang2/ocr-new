...
- 第一个选项是 "自动" (通常带有 (A) 图标)。
- 第二个选项是 "手动" (通常带有手指标志)。
请根据哪个选项处于高亮/被选中状态来判断当前的模式。

【自动模式识别】：左侧菜单栏第一个 "自动" 选项高亮时。
...
- 绝不要输出 Markdown 代码块，不要解释你的推理过程。"""
    ...
    conn.commit()
    conn.close()
    print("F0 Prompt updated with explicit menu position instructions.")

if __name__ == "__main__":
    update_f0_prompt_menu()
