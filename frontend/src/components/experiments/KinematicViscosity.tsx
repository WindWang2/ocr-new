...
export default function KinematicViscosity({ experiment, onRefresh }: ExperimentViewProps) {
  ...
  const getAverageTime = () => {
    const readings = experiment.readings.filter(r => r.field_key === 'flow_time')
    if (readings.length === 0) return null
    const sum = readings.reduce((acc, r) => acc + (Number(r.value) || 0), 0)
    return (sum / readings.length).toFixed(2)
  }

  const calculateViscosity = (avgTime: string | null) => {
    const coeff = Number(experiment.manual_params.capillary_coeff) || 0
    if (!avgTime || !coeff) return null
    return (coeff * Number(avgTime)).toFixed(4)
  }

  const avgTime = getAverageTime()
  const viscosity = calculateViscosity(avgTime)
    ...
    <div className="space-y-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white border-2 border-gray-100 rounded-3xl p-6 shadow-sm">
           <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">平均流经时间 (τ̄)</div>
           <div className="text-4xl font-black text-gray-900 tabular-nums">
             {avgTime ?? '--'} <span className="text-sm font-medium text-gray-300 ml-1">s</span>
           </div>
        </div>
        <div className="bg-white border-2 border-brand-100 rounded-3xl p-6 shadow-sm ring-8 ring-brand-50/50">
           <div className="text-[10px] font-black text-brand-400 uppercase tracking-widest mb-1">计算运动粘度 (ν)</div>
           <div className="text-4xl font-black text-brand-600 tabular-nums">
             {viscosity ?? '--'} <span className="text-sm font-medium text-brand-200 ml-1">mm²/s</span>
           </div>
        </div>
      </div>
